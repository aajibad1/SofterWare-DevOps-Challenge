...
resource "aws_launch_configuration" "web-lc" {
  ...
  user_data = <<-EOF
              ...
              sudo git clone https://github.com/aajibad1/SofterWare-DevOps-Challenge.git
              cd Application/calibrate_app && sudo pip3 install -r requirements.txt && python3 manage.py makemigrations && python3 manage.py migrate && python3 manage.py collectstatic
...
resource "aws_autoscaling_group" "app" {
  ...
  vpc_zone_identifier = ["${element(aws_subnet.web.*.id,count.index)}"]

  load_balancers    = aws_lb.weblb.id
